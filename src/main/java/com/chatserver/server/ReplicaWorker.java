...
public class ReplicaWorker extends ComponentWorker {
	...
	public String getResponseString(String cmd) {
		...
		if (msg.equals(COMMANDS.COPYMESSAGES.getValue())) {
			...
			// convert this json to List<Message> and apply on the existing
			// MessageStore
			try {
				...
				for (String consumer : messages.keySet()) {
					for (Message message : messages.get(consumer)) {
						MessageStore.getMessageStore().saveMessagesLocal(message.getProducer(), message.getConsumer(), message.getTimestamp(), message.getMessage());
					}
				}

				return "success-" + updateId + "-" + Configuration.getConfiguration().getBrokerId() + ":" + Configuration.getConfiguration().getReplicaPort() + "-" + messages.size();
		...
		/*
		 * handle the UPDATEOFFSETS command from sibling chatServers this is
		 * triggered when a consumer reads messages from some other chatServer,
		 * and we need to update the last read offset value to this chatServer as well
		 */
			...
			try {
				...
				for (String consumer : offsetsPerConsumer.keySet()) {
					MessageStore.getMessageStore().markReadLocal(consumer, offsetsPerConsumer.get(consumer));
				}

				return "success-" + updateId + Configuration.getConfiguration().getBrokerId() + ":" + Configuration.getConfiguration().getReplicaPort();
			} catch (JsonParseException e) {
				e.printStackTrace();
				return "failed-" + updateId + "-" + Configuration.getConfiguration().getBrokerId() + ":" + Configuration.getConfiguration().getReplicaPort();
			} catch (JsonMappingException e) {
